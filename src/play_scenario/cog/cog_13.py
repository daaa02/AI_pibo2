...
def Play_World(user_name):
    ...
    def start():
        ...
        while True:
            time.sleep(1)
            text_to_speech(f"먼저 물건을 숨길 사람과 맞출 사람을 정해야 해. {wm.word(user_name, 0)}가 먼저 맞춰보자.")
            text_to_speech(f"친구가 물건을 숨기는 동안 {wm.word(user_name, 0)}는 눈을 감고 있어.")
            break
        ...
        behavior_list.do_suggestion_L()
        while True:
            text_to_speech(f"이번엔 {wm.word(user_name, 0)}가 물건을 숨겨보자. 물건을 숨기는 동안 친구는 눈을 감아줘.")
            break

        behavior_list.do_waiting_B()
        while True:
            text_to_speech(f"{wm.word(user_name, 0)}는 다 숨기고 나면 다 숨겼어 라고 말해줘.")
