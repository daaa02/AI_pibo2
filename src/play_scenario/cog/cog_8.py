...
def Play_Sound(user_name):
    ...
    def start():
        ...
        while True:
            time.sleep(1)
            text_to_speech(f"먼저 {wm.word(user_name, 0)}가 소리를 내고 친구가 맞춰보기를 하자. 눈을 꼭 감고 뒤를 돌아서 소리를 집중해서 듣는 거야. ")
            break
        ...
        while True:
            text_to_speech(f"이제 역할을 바꿔보자. {wm.word(user_name, 0)}는 눈을 꼭 감고 뒤를 돌아 앉아보는거야.")
