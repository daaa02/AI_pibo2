...
#                 wait_for(number != 0)
        ...
        behavior_list.do_suggestion_L()
        while True:
            text_to_speech(f"이번엔 집안의 여러 물건의 길이를 재보자. TV, 식탁, 서랍장 처럼 {wm.word(user_name, 0)}가 재보고 싶은 물건을 찾아봐.")
            break
...
#                 wait_for(number != 0)
        ...
        behavior_list.do_waiting_C()
        while True:
            text_to_speech(f"{wm.word(user_name, 0)}가 재보고 싶은 물건을 찾았으면 찾았어 라고 말하고 그 앞으로 파이보를 옮겨줘.")
            time.sleep(1)
...
#                 wait_for(number != 0)
        ...
        behavior_list.do_suggestion_S()
        while True:
            text_to_speech(f"이번엔 {wm.word(user_name, 0)}랑 50cm 떨어진 위치랑 80cm 떨어진 위치에 테이프를 붙여보자")
            break
