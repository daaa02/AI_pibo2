...
def Play_Ghost(user_name):
    ...
    # 2.3 놀이 시작
    def start():
        behavior_list.do_joy_A()#춤=dance=joy 정지=stop=sad
        while True:
            #음악재생
            ...
            if 1<=i<3:    
                ...
                while True:
                    ...
                    if answer == 'AGAIN':
                        ...
                        while True:
                            text_to_speech(f"열심히 춤춘 {wm.word(user_name, 0)}가 최고야~ 정말 신났어!")
                            break
                    break
    ...
    while True:
        text_to_speech(f"그랬구나. 그래도 {wm.word(user_name, 0)}가 춤을 잘 춰서 파이보는 정말 재미있었어!")
        break
    ...
    behavior_list.do_praise_S()
    while True:
        text_to_speech(f"{wm.word(user_name, 0)}는 최고의 유령댄서였어~")
