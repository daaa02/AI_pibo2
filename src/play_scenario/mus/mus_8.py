...
def Play_Chef(user_name):
    ...
    def start():
        ...
        while True:
            ...
            time.sleep(1)
            text_to_speech(f"{wm.word(user_name, 0)}가 다리가 더 튼튼해진 것 같아!")
            break
    ...
    behavior_list.do_question_S()
    while True:
        text_to_speech(f"{wm.word(user_name, 0)}는 어땠어?")
    ...
    while True:
        ...
        time.sleep(1)
        text_to_speech(f"{wm.word(user_name, 0)}가 힘들어도 끝까지 하는 모습이 보기 좋았어~")
        break
