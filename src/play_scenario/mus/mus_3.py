...
def Play_Clothespin(user_name):
    ...
    def start():
        ...
        while True:
            time.sleep(1)
            text_to_speech(f"{wm.word(user_name, 0)}는 어떤 모양을 만들고 싶어?")
    ...
    while True:
        text_to_speech(f"{wm.word(user_name, 0)}는 뭐가 가장 기억에 남아?")
    ...
    behavior_list.do_joy_A()
    while True:
        text_to_speech(f"알려줘서 고마워. {wm.word(user_name, 0)}랑 노는 건 항상 너무 재밌어~")
        time.sleep(1)
        break
