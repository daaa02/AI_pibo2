...
class Dictionary:
    def __init__(self):
        self.Yes = ['yes', '할 수 있어' '네', '예', '응', '어', '있어', '좋아', '좋은', '좋다', '그래', '맞아', '알았어', '알겠어', '당연', '됐어']

        self.No = ['no', '아니', '안', '별로', '글쎄', '싫어', '싫', '못 하', '못 하겠어', '못해', '없었어', '없어', '없네',
                   '없는', '몰라', '모르', '몰라', '그만']

        self.Done = ['done', '맞췄어', '준비됐어', '완료', '됐어', '했어', '하자', '할래', '왔어', '렸어', '시작', '끝', '다 만들었어', '다 그렸어', '다 오렸어', '다 올렸어', '성공', '이겼', '이긴', '승리', '왔어']
    ...
    def nlp_answer(self, user_said, dic):
        ...
        for n in range(len(dic.Again)):
            if dic.Again[n] in user_said:
                answer = 'AGAIN'
        for m in range(len(dic.Chance)):
            if dic.Chance[m] in user_said:
                answer = 'CHANCE'
        print(answer)
        return answer
